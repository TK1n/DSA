    ...
    if (deleteUserData != 0) {
        for (int i = 0; i < count; i++) {
            if (std::is_pointer<T>::value) {
                ...
                // Do nothing, as T is not a pointer type
            }
        }
    }
    delete[] data;
...
void XArrayList<T>::checkIndex(int index)
{
    ...
    {
        throw std::out_of_range("Index: " + std::to_string(index) + ", Size: " + std::to_string(this->count));
    }
}
template <class T>
void XArrayList<T>::ensureCapacity(int index)
{
    ...
    {
        throw std::out_of_range("Index: " + std::to_string(index) + ", Size: " + std::to_string(capacity));
    }
